...
switch (process.argv.filter((v) => v.startsWith("-"))[0]) {
  ...
  case "-h":
    return console.log(
      chalk.blue`${fs.readFileSync(`${__dirname}/src/helpmsg`).toString()}`
    );
  case "--version":
  case "-v":
    return exec("npm view cepta version", (error, stdout, _) =>
      error
        ? console.log(chalk.red`Error checking version\n${error}`)
        : console.log(
            chalk.blue`Cepta version: ${
  ...
  case "--update":
  case "-u":
    return exec("npm install -g cepta", (error, ..._) =>
      error
        ...
        if (answers.project) {
          ...
          exec(
            ...
            (error, ..._) => {
              ...
              fs.readFile(
                answers.directory ? `./${answers.projectName}/.env` : `./.env`,
                "utf8",
                function (err, data) {
                  ...
                  fs.writeFile(
                    answers.directory
                      ...
                      : `./.env`,
                    data.replace(/1945/g, answers.projectPort),
                    "utf8",
                    function (err) {
                      if (err) errorhandler(err);
                    }
              ...
              fs.readFile(
                answers.directory
                  ? `./${answers.projectName}/tests/app.test.js`
                  : `./tests/app.test.js`,
                "utf8",
                function (err, data) {
                  if (err) errorhandler(err);
                  fs.writeFile(
                    answers.directory
                      ...
                      : `./tests/app.test.js`,
                    data.replace(
                      /const port = process.env.PORT || '80';/g,
                      `const port = process.env.PORT || '${answers.projectPort}';`
                    ),
                    "utf8",
                    function (err) {
                      if (err) errorhandler(err);
              ...
              console.clear();
              console.log(
                chalk.green`🔥 Your project has been created successfully`
              );
              console.log(" ");
              console.log(
                `To to start your project on port ${
                  ...
                  answers.directory ? `cd ${answers.projectName} && ` : ""
                }npm start`}`
              );
              console.log(" ");
              console.log(
              ...
              exec(
                answers.directory ? `code ${answers.projectName}` : `code .`,
                (error, ..._) => {
                  if (error) errorhandler(error);
                }
              );
              exec(`npm i -g nodemon`, (error, ..._) => {
                if (error) errorhandler(error);
              });
            }
          );
