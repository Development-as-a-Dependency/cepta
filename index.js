...
if (process.argv[2] === 'create' || process.argv[2] === 'c') {
  ...
  exec('npm i -g cepta', (err, stdout, stderr) => {
    if (err) {
      console.log("Something went wrong when updating cepta. Please update it manually.")
      return;
    }
    console.log("Cepta updated successfully. You can now run 'cepta help' to see the help message.");
  });
} else {
